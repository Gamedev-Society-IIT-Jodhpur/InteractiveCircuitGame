...
public class SIUnits : MonoBehaviour
{
    ...
    public static string NormalizeRounded(double v, int d, string u)
    {
        ...
        if (va < 1E-6) return Math.Round(v * 1E9, d) + "n" + u;  // nano
        if (va < 1E-3) return Math.Round(v * 1E6, d) + "u" + u;  // micro
        if (va < 1) return Math.Round(v * 1E3, d) + "m" + u;  // milli
        if (va < 1E3) return Math.Round(v, d).ToString() + u;
        if (va < 1E6) return Math.Round(v * 1E-3, d) + "K" + u; // kilo
        if (va < 1E9) return Math.Round(v * 1E-6, d) + "M" + u; // mega
        if (va < 1E12) return Math.Round(v * 1E-9, d) + "G" + u; // giga
        return Math.Round(v * 1E-12, d) + "T" + u;               // tera
    }
