...
public class ValidateScript : MonoBehaviour
{
    ...
    public bool CheckSpecs()
    {
        ...
        dc1.ExportSimulationData += (sender, exportDataEventArgs) =>
        {
            ...
            if (Math.Abs(Math.Abs(exportDataEventArgs.GetVoltage(node1, node2)) - 6) <= 1 &&  Math.Abs(exportDataEventArgs.GetVoltage(node1, node2)) * Math.Abs(currentExport1.Value) <=1.2)
            {
                passed = true;
            }
        };
        dc2.ExportSimulationData += (sender, exportDataEventArgs) =>
        {
            ...
            if (passed == true)
            {
                if (Math.Abs(Math.Abs(exportDataEventArgs.GetVoltage(node1, node2)) - 6) <= 1 && Math.Abs(exportDataEventArgs.GetVoltage(node1, node2)) * Math.Abs(currentExport2.Value) / power >= 0.6 && Math.Abs(exportDataEventArgs.GetVoltage(node1, node2)) * Math.Abs(currentExport2.Value)<=1.2)
                {
                    passed = true;
                }
                else
